...
def init_arduino():
    ...
    """
    try:
        arduino = serial.Serial('COM3', 4800)  # Substitua 'COM17' pela porta correta
...
def handle_keyboard_input(
    arduino, midi_out, active_keys, active_bars, bpm, midi_playing, midi_events,
    event_index, midi_start_time, midi_files, selecting_midi, midi_file,
    metronome_sound_on, submenu_visible, is_paused, progress_position
):
    ...
    if arduino and arduino.in_waiting > 0:
        try:
            ...
            pressed_keys = eval(data)  # Converte string recebida para lista (ex: "[1, 3]" -> [1, 3])
            ...
            if pressed_keys == []:
                # Se a lista é vazia, libera todas as teclas
                ...
                # Converte as teclas recebidas para notas MIDI
                current_notes = set(note_mapping[key] for key in pressed_keys if key in note_mapping)

                # Pressiona teclas que estão na lista recebida e não estão ativas
                for note in current_notes - active_keys:
                    midi_out.send_message([0x90, note, 110])  # Note on
                    active_keys.add(note)
                    if note not in active_bars:
                        active_bars[note] = []
                    active_bars[note].append((time.time(), None))

                # Libera teclas que não estão na lista recebida mas estão ativas
                for note in active_keys - current_notes:
                    midi_out.send_message([0x80, note, 0])  # Note off
                    active_keys.remove(note)
                    if note in active_bars:
                        for i, (start_time, released) in enumerate(active_bars[note]):
            ...
            print(f"Erro ao processar entrada do Arduino: {e}")
            ...
            while event_index < len(midi_events) and current_time - midi_start_time >= midi_events[event_index][0]:
                ...
                if not msg.is_meta:
                    ...
                    if msg.type == 'note_on' and msg.velocity > 0:
                        # Adiciona a nota ao conjunto de teclas ativas
                        ...
                        # Remove a nota do conjunto de teclas ativas
                    ...
                    # Envia a lista de LEDs correspondentes às notas ativas via serial
                    leds_to_light = [note_to_led[note] for note in active_keys if note in note_to_led]
                    arduino.write(f"{leds_to_light}\n".encode())  # Envia a lista como string para o Arduino
